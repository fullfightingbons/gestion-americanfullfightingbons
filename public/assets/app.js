...
function vFeedbackListe(){
  ...
  <tbody>${sorted.map(c=>{
    ...
    ${canWrite&&c.statut==='active'?`<button class="btn sm" style="margin-left:4px" onclick="ouvrirEnvoi('${c.id}')">📨 Inviter</button><button class="btn sm" style="margin-left:4px" onclick="envoyerInvitesEnAttente('${c.id}')">📤 Envoyer</button><button class="btn sm" style="margin-left:4px" onclick="relancerRecensementSaison('${c.id}','${c.exercice_id||''}')" ${c.exercice_id?'':'disabled'} title="${c.exercice_id?"Recense à nouveau les adhérents de cet exercice (utile après correction de rattachement) et envoie aux nouveaux/manquants":"Cette campagne n'est reliée à aucun exercice (exercice_id manquant) — impossible de recenser automatiquement"}">🔁 Recenser &amp; envoyer</button><button class="btn sm" style="margin-left:4px" onclick="cloturerCampagne('${c.id}')">⏹ Clôturer</button>`:''}
...
function vFeedbackDetail(camp){
  ...
  ${canWrite&&camp.statut==='active'?(()=>{const nonRepondants=recs.filter(r=>r.envoye&&!r.repondu).length;return`<button class="btn" onclick="ouvrirEnvoi('${camp.id}')">📨 Inviter des adhérents</button><button class="btn" onclick="envoyerInvitesEnAttente('${camp.id}')">📤 Envoyer les invitations en attente</button><button class="btn" onclick="relancerRecensementSaison('${camp.id}','${camp.exercice_id||''}')" ${camp.exercice_id?'':'disabled'} title="${camp.exercice_id?"Recense à nouveau les adhérents de cet exercice (utile après correction de rattachement) et envoie aux nouveaux/manquants":"Cette campagne n'est reliée à aucun exercice (exercice_id manquant) — impossible de recenser automatiquement"}">🔁 Recenser &amp; envoyer</button>${nonRepondants?`<button class="btn bwarn" onclick="relancerNonRepondants('${camp.id}',${nonRepondants})">📣 Relancer (${nonRepondants})</button>`:''}<button class="btn" onclick="cloturerCampagne('${camp.id}')">⏹ Clôturer</button>`;})():''}
